...
- Product tests are Testing Library journeys in one file. Kernel tests already exist and must stay excluded from `npm test`. Persist: remount `<App />` and assert the data survived; do not read `localStorage` keys.
