...
**Seed primitives, 31 August 2026** (journey helpers, `RecordForm` validation, Pico). Four ideas after the `tests_run` shape fix: `development-idea` ×2, pantry, timer — **4/4 `success`, 12/12 harness checks green**, 7–8 journeys each, `links: []`, kernel `diff` empty. Both book runs test a validation journey that was impossible before. Helper adoption is total: **0 locally-defined helpers and 0 hand-built filter strings** in both record-keeping runs, against 2–3 and 0–2 before.

Cost is the honest cost: median **€0.0612** against €0.0443 for the same four ideas before, roughly +38%, with journeys up (median 7 → 8) and timer doing 8 journeys where it used to do 5. Still far under the plain seed's €0.0818. Recorded as a real regression on the ranked term, not waved through.

The first attempt at this batch scored 1/4 and the cause is worth keeping: pantry and timer built working apps with green harness checks and still landed `partial`, because they wrote `{"journey", "status"}` in `tests_run` instead of `{"command", "journey", "result"}` and [`normalizeTestRun`](../src/result.ts) discards entries missing either. Growing skill step 6 by ~20 lines was enough to push the step 8 template out of reach and make the model reconstruct that JSON from memory. The entry shape is now restated inline where `tests_run` is first mentioned. **A prompt addition can break an unrelated downstream field; measure the whole pipeline, not the feature.**
