...
time is output tokens over throughput. Two things in this repository move them off the model, and
they do different jobs.

**The thinking hook decides whether a run finishes at all.** Same checkout, same idea, same model,
reasoning on vs. off — nothing else changed:

| Plain seed | Reasoning | Output tokens | Wall | Harness | Journeys |
| --- | --- | --- | --- | --- | --- |
| n=3 | on | 14,636–15,970 | 15.0 min, **all three killed** | 0/9 | 0 |
| n=3 | off | 12,998–17,336 | 12.8–14.0 min | 9/9 | 5–12 |

**The seed decides how much room is left once it does.** Same idea, same model, reasoning off in
both:

| Arm | Output tokens | Wall | Cost | Journeys |
| --- | --- | --- | --- | --- |
| **Graph seed**, n=5 | 2,992–4,575 | 2.8–4.9 min | €0.0248–0.0341 | 6–11 |
| **Plain seed**, n=3 | 12,998–17,336 | 12.8–14.0 min | €0.068–0.089 | 5–12 |

Both arms pass, at roughly 4x the tokens and 2.6x the cost apart. What separates them is margin
against the 15-minute wall: the plain arm's slowest run leaves 1.0 minute; the graph arm's leaves
10.1. Berget's measured throughput varies roughly 3x by time of day, and every run above was taken
in a good window — a plain-seed run that passes at 14.0 minutes in the evening does not pass at
midday. Put as a tolerance, the plain arm survives a 1.07x slowdown before the wall; the graph arm
survives 3.06x.

Across nine further ideas, eleven of eleven runs succeeded at `thinking_chars: 0` — deliberately
against a *wrong* local model configuration, because local configuration does not ship and the
extension has to carry judging day alone. Two of those ideas had never once passed on this model
before.
...
**[docs/approach.md](docs/approach.md)** has the argument, the confounds in both tables, the other
two hook measurements, and what is not measured.
...
The shipped `protected-paths` extension also breaks the loop at its source: a repeated `bash` command is refused after `CHALLENGE_REPEAT_LIMIT` attempts (default 12) with an explanation the model can act on. Healthy runs repeat their most-repeated command 2–7 times, which is ordinary repair, so the limit sits above that band and only `bash` is metered. Repeats are counted on the command's first line rather than the whole invocation, because the loop that actually occurs is a model debugging through a heredoc: the first line is constant while the body changes every time. One observed run repeated that shape 34 times without the counter passing one. The first line is the conservative normalization — it collapses heredocs while leaving single-line commands that differ in their arguments distinct, since over-blocking a healthy repair loop is worse than missing a loop the wall clock catches anyway.

The same extension makes `CHALLENGE_THINKING=off` actually reach the model. Pi encodes thinking controls differently per provider, and on an OpenAI-compatible vLLM backend only `chat_template_kwargs` is forwarded into the chat template — a top-level `thinking` or `thinking_token_budget` field is accepted and then dropped. So when thinking is off and the request is `openai-completions`, a `before_provider_request` hook merges `enable_thinking: false` into `chat_template_kwargs`. Measured on one model this took reasoning from 18,733 characters to 0 and output tokens from 7,087 to roughly 3,700, which decides whether a run finishes inside the wall. The hook never fires for a requested reasoning level, never fires on other APIs, merges rather than replaces, and passes any unrecognised payload through untouched; set `CHALLENGE_THINKING_COMPAT=off` to disable it entirely.
